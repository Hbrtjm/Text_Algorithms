def text_counter():
    ...
    char_count_no_spaces = sum( [0 if c == ' ' else 1 for c in text] )
    ...
    vowel_count =  sum( [1 if c in vowels else 0 for c in text] )

    # TODO: Zlicz spółgłoski w tekście
    consonant_count =  sum( [1 if c in consonants else 0 for c in text] )
